...
- **Navigate-Away Confirmation:** A native browser dialog warns users when they attempt to navigate away, refresh, or close the page, preventing accidental data loss.
- **Fresh Sessions:** Refreshing or reloading the page starts a completely fresh session. All browser-persisted settings (`localStorage` overrides), dynamic controls, custom prompts, batch counts, and toggle states are cleared and reset to their default configurations.
