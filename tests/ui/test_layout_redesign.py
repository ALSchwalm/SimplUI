...
def test_layout_structure():
    ...
    checkboxes = [
        c
        for c in components
        if c["type"] == "checkbox" and c["props"].get("label") == "Advanced Controls"
    ]
    ...
    btn_cols = [
        c
        for c in components
        if c["type"] == "column" and "vertical-buttons" in c["props"].get("elem_classes", [])
    ]
    assert len(btn_cols) == 1, "Should have a column with 'vertical-buttons' class"

    # Check for equal_height Row
    prompt_rows = [
        c for c in components if c["type"] == "row" and c["props"].get("equal_height") is True
    ]
    ...
    sidebar = next(
        c for c in components if c["type"] == "column" and c["props"].get("visible") is False
    )
    ...
    workflow_dropdowns = [
        c
        for c in components
        if c["type"] == "dropdown" and c["props"].get("label") == "Select Workflow"
    ]
...
def test_vertical_stack():
    ...
    prompt = next(
        c for c in components if c["type"] == "textbox" and c["props"].get("label") == "Prompt"
    )
